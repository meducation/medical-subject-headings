...
module MESH
  class EntryTest < TestBase
    ...
    def test_index_is_correct_for_entire_string
      entry = Entry.new(@parent_heading, 'cardiology', :en_gb)
      matches = entry.match_in_text("Cardiology", "cardiology")
      assert_equal [0,9], matches[0][:index]
    end

    def test_index_is_correct_for_string_with_whitespace
      entry = Entry.new(@parent_heading, 'cardiology', :en_gb)
      matches = entry.match_in_text("  Cardiology  ", "  cardiology  ")
      assert_equal [2,11], matches[0][:index]
    end

    def test_index_is_correct_for_string_surrounded_by_with_words
      entry = Entry.new(@parent_heading, 'cardiology', :en_gb)
      matches = entry.match_in_text(" A Cardiology Hero ", " a cardiology hero ")
      assert_equal [3,12], matches[0][:index]
    end
